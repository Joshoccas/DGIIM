/**
 * 
 * Ejemplo que ilustra el uso de <tt>declare target</tt>: daxpbyz32_ompoff2.c
 * 
 */
...
#define MAX 50331648  /**< tamaño máximo vectores dinámicos */ //100663296 
#pragma omp declare target
 float x[MAX], y[MAX], z[MAX], p;  int N; /**< los vectores deben ser variables globales para usar declare target */
#pragma omp end declare target

/**
 * @file  daxpbyz32_ompoff2.c 
 * @brief _Simple precision-real Alpha x Plus Beta y (z = Alpha * x + Beta * y) con OpenMP offload, usa <tt>declare target</tt>_
 ...
 * 
 * - <tt>declare target, target update</tt>
 * 
...
t2 = omp_get_wtime();
#pragma omp target teams distribute parallel for //map(to: N, p, x[:N])
    for (int i = 0; i < N; i++)
        z[i] = p * x[i];
...
#pragma omp target update from(z[:N])
    ...
    if (N < 11)
    {
        printf(" * \tTiempo: ((Reserva+inicialización) host %11.9f) + (target data update: %11.9f) + (target1 %11.9f) + (host actualiza %11.9f) + (target data update %11.9f) + (target2 %11.9f) + (target data update %11.9f)= %11.9f\t / Tamaño Vectores:%d\n",
    ...
    {
        printf(" * \tTiempo: ((Reserva+inicialización) host %11.9f) + (target data update %11.9f) + (target1 %11.9f) + (host actualiza %11.9f) + (target data update %11.9f) + (target2 %11.9f) + (target data update %11.9f)= %11.9f\t / Tamaño Vectores:%d\t/ alpha*x[0]+beta*y[0]=z[0](%8.6f*%8.6f+%8.6f*%8.6f=%8.6f) / / alpha*x[%d]+beta*y[%d]=z[%d](%8.6f*%8.6f+%8.6f*%8.6f=%8.6f) /\n",
